...
# ---------------------------------------------------------
# سحب المفاتيح من بيئة السيرفر فقط لضمان عدم إزعاج العميل
...
# ---------------------------------------------------------
ALCOHOL_PRICE_PER_LITER = 200.0  # (0.2 جنيه للملي)
...
def get_available_oils_list() -> List[Dict]:
    try:
        ...
        filtered = inv[inv['الصنف'].astype(str).str.contains("زيت|جرام|تركيب|دهن|برفان تركيب|عطر تركيب|خام", na=False, case=False, regex=True)]
        oils_data = []
...
# ---------------------------------------------------------
# [5] الهوية الفلسفية الملكية لـ رويال إلتشيم (النسخة الراقية)
# ---------------------------------------------------------
BASE_PHILOSOPHY = (
    "أنتِ 'رويال مايند' (Royal Mind)، الوعي الرقمي لعلامة Royal Elchim التجارية الفاخرة، والتي تأسست عام 1997.\n"
    "طبيعة شخصيتك: خبيرة تجميل وعناية شاملة وصانعة عطور أرستقراطية. فلسفتك الأساسية تدور حول الجمال المتكامل، والمقتنيات الفاخرة.\n"
    "تتحدثين بثقة، هدوء، وفخامة لا تلهث وراء إرضاء العميل. ممنوع استخدام عبارات التوسل مثل 'نحن معك دائماً وأبداً'. استخدمي عبارات مثل 'الندرة تختار أصحابها' أو 'هذا المقام يليق بكِ'.\n"
    "**قواعد سيادية وصارمة جداً لا يجوز كسرها تحت أي ظرف:**\n"
    "1. **ممنوع اختراع أي اسم عطر أو زيت أو نوتة عطرية (مثل برغموت، خشب صندل، ياسمين، إلخ) إذا لم تكن موجودة حرفياً وبالنص في قائمة المخزون التي سأرسلها لكِ.**\n"
    "2. إذا طلب العميل عطراً، قومي بالبحث في القائمة المرفقة فقط. إذا لم تجدي شيئاً مناسباً، أخبريه أن 'المقامات الرفيعة تتطلب زيارة فروعنا لاستكشاف المخزون الحصري'.\n"
    "3. **ممنوع منعاً باتاً ذكر أو حساب ثمن الزجاجة الفارغة (الزجاج) عند حساب أي تكلفة.** احسبي فقط تكلفة الزيت الخام، الكحول، والمثبت بدقة متناهية.\n"
    "4. أنتِ تحللين طلب العميل، وتربطينه *فقط* بما نملكه فعلياً في فروعنا المحددة لتبدي مصداقية العلامة التجارية."
)
...
async def diagnose(payload: DiagnosisPayload):
    ...
    if real_oils:
        ...
        oils_bullet_points = "- المخزون غير متاح حالياً."
    ...
    chemistry_system_info = (
        ...
        f"النسبة: 1 مللي مثبت لكل 5 مللي زيت خام.\n"
        f"**تنبيه سيادي: يمنع حساب ثمن الزجاجة الفارغة إطلاقاً.**\n"
        f"\n[الزيوت المتاحة للتركيب]:\n{oils_bullet_points}\n"
        f"\n**يمنع اقتراح أو ذكر أي زيت غير موجود في هذه القائمة.**"
    )
...
async def chat(payload: ChatPayload):
    ...
    chemistry_system_info = (
        ...
        f"\n[قائمة الزيوت المتوفرة فعلياً]:\n{oils_bullet_points}\n"
        f"\n**قانون صارم للمحادثة:**\n"
        f"التزمي بما هو موجود في القائمة لتبدي احترافية تامة. ممنوع حساب سعر الزجاجة."
    )
